...
# PATCH item
@router.patch("/{trip_id}/items/{item_id}", response_model=ItemResponse)
def partial_update_item(trip_id: int, item_id: int, updated: ItemUpdate, db: Session = Depends(get_db), token: str = Depends(oauth2_scheme)):
    username = decode_access_token(token).username
    user = user_service.get_user_by_username(db, username)
    if not user:
        raise HTTPException(status_code=401, detail="Unauthorized")

    return item_service.update_item(db, item_id, user.id, trip_id, updated)

# Delete an item from a trip
@router.delete("/{trip_id}/items/{item_id}", status_code=status.HTTP_204_NO_CONTENT)
def delete_trip_item(trip_id: int, item_id: int, db: Session = Depends(get_db), token: str = Depends(oauth2_scheme)) -> None:
    username = decode_access_token(token).username
    user = user_service.get_user_by_username(db, username)
